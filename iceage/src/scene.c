...
void init_scene(Scene *scene)
{
	...
	load_model(&(scene->polarbear), "data/polarbear.obj");
	load_model(&(scene->igloo), "data/igloo2.obj");
	load_model(&(scene->snow), "data/snow.obj");

	scene->texture_id_ground = load_texture("data/snow.jpg");
	scene->texture_id_igloo = load_texture("data/snow.jpg");
	scene->texture_id_wall = load_texture("data/snowwall.jpg");

	scene->texture_id_polarbear = load_texture("data/szorme.jfif");
...
void draw_scene(const Scene *scene)
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		glBindTexture(GL_TEXTURE_2D, scene->texture_id_snow);
		glPushMatrix();
		glRotatef(0, 1, 0, 0);
		glScalef(0.5f, 0.5f, 0.5f);
		glTranslatef(rand() % 100, rand() % 100, rand() % 100);
		draw_model(&(scene->snow));
		glPopMatrix();
	}
	...
	// Draw model: igloobig
	glBindTexture(GL_TEXTURE_2D, scene->texture_id_igloo);
	glPushMatrix();
	glRotatef(90, 1, 0, 0);
	glScalef(0.5f, 0.5f, 0.5f);
	glTranslatef(35, 0, 25);
	draw_model(&(scene->igloo));
	glPopMatrix();

	// Draw model: igloosmall
	glBindTexture(GL_TEXTURE_2D, scene->texture_id_igloo);
	glPushMatrix();
	glRotatef(90, 1, 0, 0);
	...
	// Draw model: polarbear
	glBindTexture(GL_TEXTURE_2D, scene->texture_id_polarbear);
	glPushMatrix();
	glRotatef(90, 1, 0, 0);
	glTranslatef(38, 0, 28);
	glScalef(0.3f, 0.3f, 0.3f);
	draw_model(&(scene->polarbear));
	glPopMatrix();
